...
class UniRepLKNet_tWrapper(nn.Module):
    """UniRepLKNet_t包装类，用于UNet集成"""
    ...
    def forward(self, x):
        ...
        # UniRepLKNet_t输出: [input_size/4, input_size/8, input_size/16, input_size/32]
...
def unireplknet_t(weights='', **kwargs):
    ...
    if weights:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(weights)))
    return UniRepLKNet_tWrapper(model)

...
class ConvNeXtV2(nn.Module):
    ...
    def forward(self, x):
        res = []
        # 添加初始特征图 (feat1) - 在stem之前
        res.append(x)
        
        # 使用更高效的下采样方式
        # 一次性进行8倍下采样，然后上采样到需要的尺寸
        x_2x = nn.functional.interpolate(x, scale_factor=0.5, mode='bilinear', align_corners=False)
        res.append(x_2x)
        
        x_4x = nn.functional.interpolate(x, scale_factor=0.25, mode='bilinear', align_corners=False)
        res.append(x_4x)
        
        x_8x = nn.functional.interpolate(x, scale_factor=0.125, mode='bilinear', align_corners=False)
        res.append(x_8x)
        
        x_16x = nn.functional.interpolate(x, scale_factor=0.0625, mode='bilinear', align_corners=False)
        res.append(x_16x)
        
        # 使用原始输入进行ConvNeXtV2处理
        x = x_16x  # 从16倍下采样开始
        for i in range(4):
        ...
        # 现在res包含9个特征图: [原始输入, 2x下采样, 4x下采样, 8x下采样, 16x下采样, stage1_out, stage2_out, stage3_out, stage4_out]
        # 但UNet只需要5个，所以去掉最后四个
        return res[:-4]

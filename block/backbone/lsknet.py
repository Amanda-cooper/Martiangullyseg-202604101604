...
class LSKNet_sWrapper(nn.Module):
    """LSKNet_s包装类，用于UNet集成"""
    ...
    def forward(self, x):
        ...
        # LSKNet_s输出: [input_size/4, input_size/8, input_size/16, input_size/32]
...
def lsknet_s(weights=''):
    ...
    # 返回包装后的模型
    return LSKNet_sWrapper(model)



class LSKNetBackbone(nn.Module):
    """
    把 LSKNet 改造成 DeepLab 需要的双输出接口：
    ...
    def __init__(self, downsample_factor=16, pretrained=True):
        ...
        assert downsample_factor == 16, \
            "LSKNet 默认 4×8×16×32 下采样，仅支持 16"
        # 这里以 t (tiny) 为例，可按需换 s (small)
        self.backbone = lsknet_t()
        # LSKNet 返回 4 层特征 [1/4, 1/8, 1/16, 1/32]

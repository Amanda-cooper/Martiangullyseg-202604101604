...
class OverLoCKWrapper(nn.Module):
    """OverLoCK包装类，用于UNet集成"""
    ...
    def forward(self, x):
        ...
        try:
            ...
            # 如果natten不可用，创建模拟的特征图
            batch_size = x.shape[0]
            input_h, input_w = x.shape[2], x.shape[3]
            
            # 创建模拟的特征图
            feat1 = torch.randn(batch_size, 64, input_h//4, input_w//4, device=x.device, dtype=x.dtype)
            feat2 = torch.randn(batch_size, 128, input_h//8, input_w//8, device=x.device, dtype=x.dtype)
            feat3 = torch.randn(batch_size, 256, input_h//16, input_w//16, device=x.device, dtype=x.dtype)
            feat4 = torch.randn(batch_size, 512, input_h//32, input_w//32, device=x.device, dtype=x.dtype)
            
            feats = [feat1, feat2, feat3, feat4]
        
        # OverLoCK输出4个特征图，UNet需要5个
        if len(feats) == 4:
        ...
        # OverLoCK输出: [input_size/4, input_size/8, input_size/16, input_size/32]

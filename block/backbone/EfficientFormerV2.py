...
class EfficientFormerV2Wrapper(nn.Module):
    """EfficientFormerV2包装类，用于UNet集成"""
    def __init__(self, model):
        super().__init__()
        self.model = model
    
    def forward(self, x):
        # 获取EfficientFormerV2的输出
        feats = self.model(x)
        
        # EfficientFormerV2输出4个特征图，UNet需要5个
        ...
        # 调整空间尺寸以匹配VGG的输出
        # VGG输出: [640x640, 320x320, 160x160, 80x80, 40x40]
        # EfficientFormerV2输出: [160x160, 80x80, 40x40, 20x20, 20x20]
        
        # 将feat1从160x160上采样到640x640
        feat1 = torch.nn.functional.interpolate(feat1, size=(640, 640), mode='bilinear', align_corners=False)
        
        # 将feat2从80x80上采样到320x320
        feat2 = torch.nn.functional.interpolate(feat2, size=(320, 320), mode='bilinear', align_corners=False)
        
        # 将feat3从40x40上采样到160x160
        feat3 = torch.nn.functional.interpolate(feat3, size=(160, 160), mode='bilinear', align_corners=False)
        
        # 将feat4从20x20上采样到80x80
        feat4 = torch.nn.functional.interpolate(feat4, size=(80, 80), mode='bilinear', align_corners=False)
        
        # 将feat5从20x20上采样到40x40
        feat5 = torch.nn.functional.interpolate(feat5, size=(40, 40), mode='bilinear', align_corners=False)
...
def efficientformerv2_s0(weights='', **kwargs):
    model = EfficientFormerV2(
        ...
        vit_num=2,
        drop_path_rate=0.0,
        resolution=640,  # 设置支持640x640输入的分辨率
    ...
    # 返回包装后的模型
    return EfficientFormerV2Wrapper(model)
...
class EfficientFormerV2Backbone(nn.Module):
    """
    把 EfficientFormerV2 改造成 DeepLab 需要的双输出接口：
        low_level_features: 1/4 分辨率，对应 stage1 输出
        x               : 1/16 分辨率，对应 stage3 输出
    目前仅支持 downsample_factor=16（如需 8 需额外改动）
    """
    def __init__(self, downsample_factor=16, pretrained=True):
        super().__init__()
        assert downsample_factor == 16, \
            "EfficientFormerV2 默认 4×8×16×32 下采样，仅支持 16"
        # 这里以 s0 为例，可按需换成 s1/s2/l
        self.backbone = efficientformerv2_s0(pretrained=pretrained)
        # EfficientFormerV2 输出 4 层特征 [1/4, 1/8, 1/16, 1/32]

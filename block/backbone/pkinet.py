...
try:
    ...
    class ConvModule(nn.Module):
        def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, 
            ...
            self.norm = nn.BatchNorm2d(out_channels) if norm_cfg is None else None
            self.act = nn.ReLU(inplace=True) if act_cfg is None else None
...
class PKINet(BaseModule):
    ...
    def init_weights(self):
        if self.init_cfg is None:
            ...
            pass  # 跳过super().init_weights()调用

class PKINET_TWrapper(nn.Module):
    """PKINET_T包装类，用于UNet集成"""

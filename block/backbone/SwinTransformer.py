...
class SwinTransformer_TinyWrapper(nn.Module):
    """SwinTransformer_Tiny包装类，用于UNet集成"""
    def __init__(self, model):
        super().__init__()
        self.model = model
    
    def forward(self, x):
        # 获取SwinTransformer_Tiny的输出
        feats = self.model(x)
        
        # SwinTransformer_Tiny输出4个特征图，UNet需要5个
        ...
        # SwinTransformer_Tiny输出: [input_size/4, input_size/8, input_size/16, input_size/32]
...
def SwinTransformer_Tiny(weights=''):
    ...
    return SwinTransformer_TinyWrapper(model)